...
async function handleLongAskFlow(dsFmt) {
  ...
  var promptBaseLong = [
    ...
    '3. ACCUMULATION TYPE matters for multi-month periods. Check accumulation_type in the resolved tree for the node you are querying:',
    '   - cumulative → SUM over ALL rows in the period: SUM(CASE WHEN <period> AND <hier> THEN ' + valueCol + ' ELSE 0 END)',
    '   - point_in_time → SUM over the LATEST MONTH of the period only (balance sheet / snapshot metric).',
    '     Build the PIT filter by adding: AND (' + yf + ', ' + mf + ') = (SELECT ' + yf + ', MAX(' + mf + ') FROM ' + tbl + ' WHERE <period> AND <hier>' + CF + ' GROUP BY ' + yf + ' ORDER BY ' + yf + ' DESC LIMIT 1)',
    '     This picks the last month with data inside the requested period. NEVER use AVG for PIT — use SUM on the latest-month row.',
    '3b. For a SINGLE-month question (e.g. "Jan 2024"), both cumulative and PIT just SUM that one month — no latest-month filter needed.',
  ...
  if (!twoPassTypeLong) {
    ...
    '',
      '## PERIOD NAMING',
    'The user may have asked about "last quarter" or "last year" — in your answer, always use the resolved period label: "' + periodUsed + '". Never parrot the user\'s phrasing.',
    '',
      '## OUTPUT — JSON only',
