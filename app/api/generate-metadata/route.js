...
'## HIERARCHY (inheritance model)',
    'The hierarchy uses INHERITANCE. Leave favorable_direction, accumulation_type,',
    'business_priority, and unit BLANK on descendants — PRISM walks up the parent chain to inherit.',
    '',
    'Set these fields explicitly at the TOPMOST LEVEL where each node represents a real, measurable metric category',
    '(a thing you could compute a meaningful number for). Do NOT set them on nodes that are pure organisational',
    'containers with no inherent metric meaning.',
    '',
    'Rules:',
    '  - Identify the topmost level per branch where the node represents a real metric category. Set all four fields on those nodes.',
    '  - Nodes above that level (pure containers) leave blank.',
    '  - Descendants below that level leave blank — they inherit.',
    '  - OVERRIDE: if a descendant genuinely differs from its ancestor (opposite favorable direction, different aggregation behaviour, etc.), set the differing field explicitly and add a review_note.',
    '',
    'accumulation_type values:',
    '  - "point_in_time" for stock/balance metrics — values represent a snapshot (e.g. headcount on a date, inventory level, account balance). Aggregation across periods uses AVG or LATEST, not SUM.',
    '  - "cumulative" for flow metrics — values accumulate over a period (e.g. sales in a month, hours worked, transactions completed). Aggregation across periods uses SUM.',
    '  - Use your judgement from the node names and context. If unclear, pick the more likely one and add a review_note.',
    '',
    '## FIELDS',
