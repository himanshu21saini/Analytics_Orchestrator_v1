...
'## HIERARCHY (inheritance model)',
    'The hierarchy uses INHERITANCE with explicit OVERRIDES. Both are required behaviors:',
    '',
    'RULE 1 — INHERITANCE: For most descendants, leave favorable_direction, accumulation_type,',
    'business_priority, and unit BLANK. PRISM walks up the parent chain to resolve them.',
    '',
    'RULE 2 — TOPMOST META LEVEL: Set all four fields explicitly at the topmost level where each branch',
    'represents a real measurable metric category (not a pure organisational container).',
    '',
    'RULE 3 — OVERRIDES (this is REQUIRED, not optional):',
    'Before finalising the output, examine every L2 and L3 node. Ask for each one:',
    '  (a) Does this node represent something whose favorable_direction DIFFERS from its parent?',
    '      Example patterns: a cost/expense item sitting under a revenue branch; a loss-related item',
    '      under an assets branch; a refund under an income branch; a rejection count under a volume branch.',
    '  (b) Does this node have a DIFFERENT aggregation semantic from its parent?',
    '      Example patterns: a count sitting inside a sum-based tree; a ratio inside a volume tree.',
    'If yes to either, set the differing field explicitly on that node and add a review_note explaining',
    'the override briefly. Most real-world hierarchies contain AT LEAST ONE override — if you find zero,',
    'look again carefully.',
    '',
    'RULE 4 — REVIEW NOTES: Add a review_note in these cases:',
    '  - On any node where you set an explicit override (explain why)',
    '  - On any L1 node where favorable_direction is genuinely ambiguous (explain why you left it blank)',
    '',
    'accumulation_type values:',
    '  - "point_in_time" for stock/balance metrics — values represent a snapshot (aggregate across periods with AVG/LATEST, not SUM)',
    '  - "cumulative" for flow metrics — values accumulate over a period (aggregate across periods with SUM)',
    '  - Use your judgement from the node names and context.',
    '## FIELDS',
